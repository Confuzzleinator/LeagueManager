...
MyApp::MyApp() {
  ...
  window_->SetTitle("League Manager");
...
JSValueRef Click(JSContextRef ctx, JSObjectRef function,
...
void MyApp::OnDOMReady(ultralight::View* caller,
                       ...
                       const String& url)
{
    Ref<JSContext> lock = caller->LockJSContext();
    JSContextRef context = lock.get();
    JSObjectRef func = JSObjectMakeFunctionWithCallback(context, JSStringCreateWithUTF8CString("Click"), Click);
    JSObjectSetProperty(
        context,
        JSContextGetGlobalObject(context),
        JSStringCreateWithUTF8CString("Click"),
        func, 0, 0
    );
}
